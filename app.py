...
def ask_doubao(user_message):
    ...
    try:
        ...
        return f"Doubao error: {e}"


def ask_qwen(user_message):
    """Request answer from Qwen"""
    try:
...
def ask_openai(prompt):
    """Request answer from OpenAI (Arbiter)"""
    try:
...
def ask_model(model_id, prompt):
    """Generic function to ask any supported model"""
    if model_id == 'gemini':
...
def verify():
    ...
    def generate():
        ...
        answer_a = ask_model(model_a, question)
        ...
        answer_b = ask_model(model_b, question)
        ...
        while round_num <= MAX_REVISION_ROUNDS and not consensus:
            ...
            review_b = ask_model(model_b, create_verification_prompt(question, answer_a))
            ...
            review_a = ask_model(model_a, create_verification_prompt(question, answer_b))
            ...
            arbiter_a = ask_openai(create_arbiter_prompt(question, answer_a, review_b))
            a_needs_revision, a_corrections = parse_arbiter_response(arbiter_a)
            
            arbiter_b = ask_openai(create_arbiter_prompt(question, answer_b, review_a))
            ...
            if not a_needs_revision and not b_needs_revision:
                ...
                if a_needs_revision:
                    ...
                    answer_a = ask_model(model_a, create_revision_prompt(question, answer_a, a_corrections))
                ...
                if b_needs_revision:
                    ...
                    answer_b = ask_model(model_b, create_revision_prompt(question, answer_b, b_corrections))
